...
function calendarExtractMonth(date: DateArray): number {
  return date[0];
}

function calendarExtractDay(date: DateArray): number {
  return date[1];
}

function calendarExtractYear(date: DateArray): number {
  return date[2];
}

function calendarUpdateYear(date: DateArray, year: number): DateArray {
  return [date[0], date[1], year];
}

function calendarLeapYearP(year: number): boolean {
  if (year < 0) year = Math.abs(year) - 1;
  return mod(year, 4) == 0 && (!(mod(year, 100) == 0) || mod(year, 400) == 0);
}

function calendarDayNumber(date: DateArray): number {
...
function calendarAbsoluteFromGregorian(date: DateArray): number | undefined {
...
function calendarGregorianFromAbsolute(date: number): DateArray {
  let d0 = date - 1,
    n400 = div(d0, 146097),
    d1 = mod(d0, 146097),
    n100 = div(d1, 36524),
...
function calendarLastDayOfMonth(month: number, year: number): number {
...
function calendarDayOfWeek(
  date: DateArray,
  absFunc = calendarAbsoluteFromGregorian
): number {
...
/*
  ...
  Author: Edward M. Reingold <reingold@cs.uiuc.edu>
*/
function calendarJulianToAbsolute(date: DateArray): number {
  let month = calendarExtractMonth(date),
    year = calendarExtractYear(date);
  return (
    calendarDayNumber(date) +
...
/* 
   ...
   Author: Edward M. Reingold <reingold@cs.uiuc.edu>
*/
const calendarPersianEpoch: number = calendarJulianToAbsolute([3, 19, 622]);
...
function calendarPersianLastDayOfMonth(month: number, year: number): number {
  if (month < 7) return 31;
  else if (month < 12 || calendarPersianLeapYearP(year)) return 30;
  return 29;
}

function calendarPersianToAbsolute(date: DateArray): number {
  let month = calendarExtractMonth(date),
    day = calendarExtractDay(date),
    year = calendarExtractYear(date);
  if (year < 0)
...
function calendarPersianFromAbsolute(date: number): DateArray {
...
function calendarPersianDayOfWeek(date: DateArray): number {
...
/* 
   ...
   Author: Edward M. Reingold <reingold@cs.uiuc.edu>
*/
const calendarIslamicEpoch: number = calendarJulianToAbsolute([7, 16, 622]);

function calendarIslamicLeapYearP(year: number): boolean {
  return (
...
function calendarIslamicDayNumber(date: DateArray): number {
  let month = calendarExtractMonth(date);
  return 30 * div(month, 2) + 29 * div(month - 1, 2) + calendarExtractDay(date);
}

function calendarIslamicToAbsolute(date: DateArray): number {
  let month = calendarExtractMonth(date),
    day = calendarExtractDay(date),
    year = calendarExtractYear(date),
    y = mod(year, 30),
...
function calendarIslamicFromAbsolute(date: number): DateArray {
...
function calendarIslamicDayOfWeek(date: DateArray): number {
...
function calendarKurdishFromAbsolutePersian(date: number): DateArray {
  return calendarKurdishFromPersian(calendarPersianFromAbsolute(date));
}

function calendarKurdishToAbsolutePersian(date: DateArray): number {
  return calendarPersianToAbsolute(calendarKurdishToPersian(date));
}

function calendarKurdishFromPersian(date: DateArray): DateArray {
  return calendarUpdateYear(date, calendarExtractYear(date) + 1321);
}

function calendarKurdishToPersian(date: DateArray): DateArray {
  return calendarUpdateYear(date, calendarExtractYear(date) - 1321);
}

function calendarKurdishFromGregorian(date: DateArray): DateArray {
  return calendarKurdishFromAbsolutePersian(
    calendarAbsoluteFromGregorian(date)!
  );
}

function calendarKurdishToGregorian(date: DateArray): DateArray {
  return calendarGregorianFromAbsolute(calendarKurdishToAbsolutePersian(date));
}

function calendarKurdishFromIslamic(date: DateArray): DateArray {
  return calendarKurdishFromAbsolutePersian(calendarIslamicToAbsolute(date));
}

function calendarKurdishToIslamic(date: DateArray): DateArray {
